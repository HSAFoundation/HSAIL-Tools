...
string dumpOperand(Operand opr)
{
    ...
    else if (OperandAddress          o = opr) { if (o.symbol()) s << "[" << o.symbol().name() << "]"; if (o.reg().deref()) s << "[" << o.reg() << "]"; if (o.offset() != 0) s << "[" << o.offset() << "]"; }
    ...
    else if (OperandImmed            o = opr) { s << "IMM(" << (unsigned)o.bytes(0); for (unsigned i = 1; i < o.byteCount(); ++i) s << ", " << (unsigned)o.bytes(i); }
...
void dumpSignalType(unsigned val) { dump("SignalType", typeX2str(val), val); }
...
void dumpMemFence(unsigned val)  { dump("MemSeg   ", (val == BRIG_MEMORY_FENCE_NONE)? "none" : memoryFenceSeg2str(val), val); }
void dumpAtmOp(unsigned val)     { dump("AtmOp    ", atomicOperation2str(val), val); }
void dumpSigOp(unsigned val)     { dump("SigOp    ", signalOperation2str(val), val); }
...
void dumpConst(unsigned val)     { dump("Const    ", 0, val); }
...
void dumpAlign(unsigned val)     { dump("Align    ", align2str(val), val); }
void dumpEqClass(unsigned val)   { dump("EqClass  ", 0, val); }
void dumpNoNull(unsigned val)    { dump("NoNull   ", 0, val); }
...
void dumpInst(Inst inst)
{
    ...
    else if (InstAtomic      i = inst) { dumpSegment(i.segment()); dumpMemOrder(i.memoryOrder()); dumpMemScope(i.memoryScope()); dumpAtmOp(i.atomicOperation()); dumpEqClass(i.equivClass()); } 
    else if (InstAtomicImage i = inst) { dumpImgType(i.imageType()); dumpCoordType(i.coordType()); dumpGeom(i.geometry()); dumpAtmOp(i.atomicOperation()); dumpEqClass(i.equivClass()); } 
    ...
    else if (InstImage       i = inst) { dumpImgType(i.imageType()); dumpCoordType(i.coordType()); dumpGeom(i.geometry()); dumpEqClass(i.equivClass()); } 
    else if (InstMem         i = inst) { dumpSegment(i.segment()); dumpAlign(i.align()); dumpEqClass(i.equivClass()); dumpWidth(i.width()); dumpConst(i.modifier().isConst()); } 
    else if (InstMod         i = inst) { dumpRounding(i.modifier().round()); dumpFtz(i.modifier().ftz()); dumpPack(i.pack()); } 
    else if (InstMemFence    i = inst) { dumpMemFence(i.segments()); dumpMemOrder(i.memoryOrder()); dumpMemScope(i.memoryScope()); } 
    else if (InstSegCvt      i = inst) { dumpSrcType(i.sourceType()); dumpSegment(i.segment()); dumpNoNull(i.modifier().isNoNull()); } 
    else if (InstSignal      i = inst) { dumpSignalType(i.signalType()); dumpMemOrder(i.memoryOrder()); dumpSigOp(i.signalOperation());  } 
    else if (InstQueue       i = inst) { dumpSegment(i.segment()); dumpMemOrder(i.memoryOrder());  } 
    else if (InstLane        i = inst) { dumpSrcType(i.sourceType()); dumpWidth(i.width());  } 
    else if (InstSourceType  i = inst) { dumpSrcType(i.sourceType()); }
    else if (InstSeg         i = inst) { dumpSegment(i.segment()); }
